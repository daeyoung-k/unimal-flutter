...
- 이미 그런 스텝이 적힌 계획을 실행할 때는 커밋 스텝을 건너뛰고, 마지막에 모아서
  한 번 커밋한다.
- 태스크 사이에는 커밋 대신 `flutter analyze` / `flutter test` 로만 검증하고 넘어간다.

> 위반 사례 (2026-07-29): 마커 사진 우선 대표 선정 작업에서 계획서의 태스크별 커밋
> 스텝을 그대로 따라 5개로 쪼개 커밋했다. 계획서가 아니라 이 문서를 따랐어야 한다.
