...
class TelVerificationScreen extends StatefulWidget {
  ...
  @override
  State<TelVerificationScreen> createState() => _TelVerificationScreenState();
}

class _TelVerificationScreenState extends State<TelVerificationScreen> {
  ...
  Future<void> _sendVerificationCode() async {
    ...
    try {
      final bool sent = await _authService
  ...
  Future<void> _verifyCode() async {
    ...
    try {
      final String msg = await _authService
          .verifyEmailTelVerificationCodeAndTelUpdate(
              _email, _tel, _verificationCode);
      if (msg == 'ok') {
        if (mounted) {
  ...
  Widget build(BuildContext context) {
    final bool phoneActive =
        _isValidPhone(_telController.text) && !_isSendLoading;
    final bool codeActive = _verificationCode.length == 6 && !_isVerifyLoading;
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                _isVerificationSent
                    // 전송 후: "재발송" 버튼 (항상 파란색, 90px)
                    ? _button(
                        label: '재발송',
                        onPressed: _isSendLoading ? null : _resendVerificationCode,
                        active: true,
                        width: 90,
                    ...
                    // 전송 전: "인증번호 전송" 버튼 (전화번호 유효하면 파란색, 120px)
                    : _button(
                        label: '인증번호 전송',
                        onPressed:
                            phoneActive ? _sendVerificationCode : null,
                        active: phoneActive,
                        width: 120,
                        child: _isSendLoading
                            ? const SizedBox(
